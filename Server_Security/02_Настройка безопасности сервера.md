...
> [!WARNING]
> Перед перезагрузкой сервера обязательно откройте новый SSH-порт в `nftables` (иначе можно потерять доступ к серверу).
> Также не забудьте изменить порт в вашем SSH-клиенте.
> Иногда в клиенте нужно создать новую сессию/вкладку, чтобы подключение прошло по новому порту.
