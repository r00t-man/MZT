...
> [!NOTE]
> `"flow": ""` в примере — не опечатка, но и не обязателен: `flow` (например `xtls-rprx-vision`) имеет
> смысл только для транспорта `tcp`/`raw` с `security: reality`. Для XHTTP его можно смело не указывать вовсе.
...
> [!IMPORTANT]
> Самая частая причина, по которой эта схема не заводится с первого раза — перепутанный `security` у
> клиента и у сервера. Они должны быть РАЗНЫМИ (не одинаковыми!), см. ниже.
...
> [!IMPORTANT]
> Если nginx принимает внешний TLS, то клиент = TLS, а Xray inbound за nginx = none.
...
> [!TIP]
> Этот раздел предполагает, что Cloudflare WARP **уже установлен** и слушает `127.0.0.1:40000` как
> SOCKS5. Если ещё не поднимал — сначала пройди отдельную инструкцию и скрипт
> [`warp-remnanode.sh`](../my-wiki/WARP-remna.md) (установка/подключение/статус WARP, генерация
> X25519-ключей), и только потом возвращайся сюда за интеграцией с этим конкретным nginx+xhttp профилем.
...
> [!NOTE]
> Если будут проблемы с резолвом — добавь секцию `dns` в конфиг ноды:
...
> [!NOTE]
> Если будут глюки (например с QUIC/HTTP-3 через WARP) — можно сузить сеть у outbound WARP до только TCP:
