...
> [!NOTE]
> В Remnawave есть директива `remnawave.injectHosts`, но она автоподставляет outbound'ы только в
> **клиентский** subscription-шаблон (XRAY_JSON для конечного пользователя), не в Config Profile,
> который реально исполняется на сервере (о нём эта шпаргалка). Для серверного каскада/балансировщика
> ручной перенос значений из VLESS-ссылки, как показано ниже, — единственный штатный способ в текущей
> версии Remnawave. Подробнее — раздел «⚠️ Про injectHosts» в
> [Балансировка remna](./Балансировка%20remna.md#-про-injecthosts--это-не-автоматизация-для-этого-балансировщика).
