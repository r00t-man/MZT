...
> [!TIP]
> Если панель Remnawave достаточно свежая, весь этот ручной перенос можно не делать — панель умеет
> сама подставлять outbound-блоки нод по UUID/remark/tag через `remnawave.injectHosts` в шаблоне.
> См. раздел «🆕 Современный способ» в [Балансировка remna](./Балансировка%20remna.md#-современный-способ-автоподстановка-нод-через-injecthosts).
> Шпаргалка ниже пригодится для понимания логики и для нетиповых случаев (кастомные конфиги, ноды не
> из этой панели).
