...
> [!IMPORTANT]
> Прежде чем запускать скрипт, надо подготовить:
...
> [!WARNING]
> Пока не запускаем — сначала пройди шаги ниже (раздел «Основная настройка»).
...
> [!IMPORTANT]
> Логи и logrotate подготовлены — теперь запускаем установку основного скрипта:
...
> [!CAUTION]
> **Блок кода ниже обрезан и не рабочий как есть.** Внутри heredoc `mobile443-apply-cache.sh`
> (начиная с `echo "===> Переписываем apply-cache под nft"`) несколько функций идут БЕЗ строки
> объявления (`имя_функции() {`) — заметно по одиноким `}` без парной открывающей скобки — а последняя
> функция (принимает `set_name`/`file`/`label`) обрывается прямо на `while IFS= read -r prefix ...; do`:
> нет тела цикла, нет `done`, нет закрывающей `}` самой функции и нет завершающего `EOF` heredoc'а.
> Копировать и запускать этот блок **как есть — нельзя**, интерпретатор либо упадёт на синтаксисе, либо
> (что хуже) `bash -Eeuo pipefail` доберётся до неполного heredoc и оставит систему в непонятном
> состоянии. Перед использованием либо восстанови полный текст из оригинального проекта
> [wh3r3ar3you/mobile443-filter](https://github.com/wh3r3ar3you/mobile443-filter), либо допиши
> недостающие части сам (функции `log()`, создание таблицы/сетов nftables, и цикл заполнения
> `nft add element` по строкам из `$file`) прежде чем сохранять в `/srv/01_trafguard.sh`.
