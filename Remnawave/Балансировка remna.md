...
## ⚠️ Про `injectHosts` — это НЕ автоматизация для этого балансировщика

В Remnawave есть корневая директива `remnawave.injectHosts` — но она относится к **другому слою**
конфигурации, не к тому, что описано в этой статье. Уточнение важно, чтобы не запутаться:

- **Balancer/routing из этой статьи** — часть **Config Profile**, который реально исполняется
  Xray-core **на самой ноде** (сервере). Это то, что физически решает, куда уйдёт трафик дальше —
  сервер-сервер каскадирование/балансировка. Согласно официальной документации Remnawave, Config
  Profile — это «чистый Xray-core JSON без каких-либо Remnawave-надстроек»: `outbounds`/`routing`
  редактируются вручную, ссылка на raw-синтаксис Xray, автоподстановки тут нет.
- **`injectHosts`** — часть **XRAY_JSON subscription template**, который относится к **Host**
  (клиентской точке входа) и обрабатывается при генерации **подписки для конечного пользователя**
  — то есть это конфиг, который скачивает **клиентское приложение**, а не то, что крутится на сервере.

Иначе говоря: если вы конфигурируете, как один ваш сервер (например, каскадная нода) сам решает,
на какую из нескольких upstream-нод отправить трафик — `injectHosts` тут не поможет, ручная правка
`outbounds`/`routing` в Config Profile ниже — единственный штатный способ в текущей версии Remnawave.
`injectHosts` пригодится в другом сценарии: если вы хотите выдать пользователю подписку в формате
XRAY_JSON, где его собственное клиентское приложение само балансирует между несколькими Host'ами —
это самостоятельная фича для клиентской стороны, не про серверный каскад.

Подробности — в официальной документации: [Xray JSON — Advanced](https://docs.rw/learn/xray-json-advanced/)
(там же про `injectHosts`) и [Server-Side Routing](https://docs.rw/learn-en/server-routing/) (про то,
что реально применимо к Config Profile — то есть к этой статье).

---

## 🛠️ Обезличенный учебный конфиг с комментариями — именно для `roundRobin`

Ниже — учебный обезличенный пример конфига балансировщика для Config Profile. Все чувствительные
данные — заглушки: UUID, ключи, shortId, адреса серверов и маскирующие домены.
