...
> [!WARNING]
> Замените в конфиге значения `ЗАМЕНИ СВОЙ ДОМЕН` и пути к сертификатам (`privkey.key`, `fullchain.pem`) на ваши реальные — иначе TLS-хендшейк не пройдёт.

> [!TIP]
> Этой схеме (XHTTP+TLS) нужен свой домен и настоящий сертификат. Если домена нет или не хочется его светить —
> альтернатива для входа в Remnawave — транспорт **REALITY** (`security: reality`, без своего домена/сертификата,
> маскируется под чужой TLS-сайт через `dest`/`serverNames`). Правила блокировки `.ru`-доменов ниже от выбора
> транспорта не зависят, их можно переиспользовать в любом профиле.
...
> [!NOTE]
> Правило `geoip:ru` в конце сработает только для соединений, которые уже пришли **по IP** (без домена в SNI) —
> при `domainStrategy: "AsIs"` Xray не резолвит домен в IP для доменных запросов, которые не подошли под более
> ранние правила, поэтому оно не «подхватывает» произвольные незнакомые `.ru`-домены, отсутствующие в списке
> выше. Если нужно, чтобы `geoip:ru` реально резолвил и блокировал ЛЮБОЙ домен с российским IP (не только из
> списка) — смените `domainStrategy` на `"IPIfNonMatch"`, но учтите: это добавляет DNS-резолв для каждого
> домена, не подошедшего под более ранние правила, то есть лишнюю задержку на большинстве обычных запросов.
