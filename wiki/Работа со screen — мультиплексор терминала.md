...
Если хочешь не запоминать команды вручную, используй `scrmenu.sh` — интерактивное CLI-меню для управления `screen`.

Что умеет скрипт кратко:
- автоматически проверяет и ставит `screen` (Ubuntu/Debian, при запуске от root);
- показывает список сессий и позволяет подключаться/переподключаться;
- создаёт новые сессии (в т.ч. detached с запуском команды в фоне);
- завершает и переименовывает сессии через меню.
...
## ⚡ Быстрый старт — если без скрипта, а чистая утилита screen
