...
> [!WARNING]
> Для работы требуется установленный `docker`. Команды удаления (`rm`, `rmi`, `prune`) нужно
> использовать осторожно. Для установки обычно нужен `root` или `sudo`. Если раньше использовались
> старые команды `control-docker`, установщик удалит их автоматически.
