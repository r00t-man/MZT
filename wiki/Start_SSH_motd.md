...
> [!NOTE]
> Скрипт рассчитан на Linux-сервер с `systemd`, для установки нужны `sudo`/`root`. Он отключает часть
> стандартных MOTD-блоков Ubuntu/Debian — штатные уведомления Canonical потом придётся включить обратно
> вручную. Если на сервере нет `resolvectl`, часть DNS-информации может показать `not found` — это
> нормально. Скрипт не ставит отдельные пакеты, использует стандартные системные утилиты.
